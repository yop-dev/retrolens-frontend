...
export function Profile() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        console.log('Starting to fetch user data for user ID:', user.id)
        ...
        try {
          ...
          if (!healthCheck.ok) {
            console.warn(`Backend server health check failed. Status: ${healthCheck.status}`)
          } else {
            console.log('API health check passed')
          }
        } catch (healthError) {
          console.warn('Backend server not responding, will use fallback profile:', healthError)
          // Use fallback immediately if backend is not accessible
        ...
        // AuthWrapper already handles user sync, so we don't need to do it here
        const username = user.username || user.firstName || user.primaryEmailAddress?.emailAddress?.split('@')[0] || `user_${user.id.slice(-8)}`
        ...
        try {
          ...
          const fetchAttempts = [
            ...
            // Attempt 2: Try by username
            async () => {
              console.log('Trying to fetch by username...')
              return await makeAuthenticatedRequest(async (token) => 
                userService.getUserByUsername(username, token)
              )
            },
            // Attempt 3: Try to create new user profile
            async () => {
              console.log('Trying to create new user profile...')
              const newUserData = {
                username: username,
                email: user.primaryEmailAddress?.emailAddress || '',
                full_name: user.fullName || `${user.firstName || ''} ${user.lastName || ''}`.trim(),
                avatar_url: '', // Don't set Clerk URL on creation, let user upload their own
              }
              return await makeAuthenticatedRequest(async (token) => 
                userService.createUser(newUserData, token)
              )
            }
          ]
          ...
          for (const attempt of fetchAttempts) {
            try {
              ...
              console.log('Fetch attempt failed:', attemptError)
              continue
            }
          }

        } catch (profileError) {
          console.warn('All profile fetch attempts failed:', profileError)
        }

        // Use fallback if no profile was retrieved
        if (!profile) {
        ...
        // Fetch cameras - using the general cameras endpoint and filtering client-side
        // since USER_CAMERAS endpoint doesn't exist in backend yet
        try {
          console.log('Fetching cameras...')
          const allCameras = await makeAuthenticatedRequest(async (token) => 
            cameraService.getAllCameras(token)
          )
          // Filter cameras for this user
          const userCameras = allCameras?.filter(camera => camera.user_id === profile.id) || []
          console.log('User cameras fetched:', userCameras)
          setUserCameras(userCameras)
        } catch (cameraError) {
          console.warn('Failed to fetch cameras:', cameraError)
          setUserCameras([])
        }

        // Fetch discussions - using the general discussions endpoint and filtering client-side
        try {
          console.log('Fetching discussions...')
          const allDiscussions = await makeAuthenticatedRequest(async (token) => 
            discussionService.getAllDiscussions(token)
          )
          // Filter discussions for this user
          const userDiscussions = allDiscussions?.filter(discussion => discussion.user_id === profile.id) || []
          console.log('User discussions fetched:', userDiscussions)
    ...
    <div className="profile-page">
      ...
      <div className="mobile-profile-header">
        ...
        {isOwnProfile ? (
          ...
          <div style={{ width: '32px' }}></div> // Placeholder to maintain layout
        )}
      </div>

      {/* Desktop Header is handled by main Header component */}
      
      <div className="profile-container">
        {/* Profile Info Section */}
        <div className="profile-info-section">
          {/* Settings button for both desktop and mobile - positioned top right of card */}
          {isOwnProfile && (
            <button 
          ...
          {/* Left Column - Avatar and Actions (Desktop) */}
            ...
            <div className="avatar-container">
              <img 
                ...
                onError={(e) => {
                  ...
                  // Fallback to Clerk image if backend avatar fails, then to default
            ...
            <div className="profile-details">
              ...
              <div className="user-meta">
                ...
                {userProfile?.instagram_url && (
                  <div className="meta-item">
                    <LinkIcon size={16} />
                    <a href={userProfile.instagram_url} target="_blank" rel="noopener noreferrer" className="social-link">
                      {userProfile.instagram_url.replace(/^https?:\/\//, '')}
                    </a>
                  </div>
                )}
              </div>
        ...
        <div className="stats-section">
          <div className="stats-grid">
            <div className="stat-item">
              ...
              <span className="stat-value">{userProfile?.camera_count || 0}</span>
            </div>
            <div className="stat-item">
              <span className="stat-label">Discussions</span>
              <span className="stat-value">{userProfile?.discussion_count || 0}</span>
            </div>
            <button 
        ...
        <div className="profile-tabs">
          ...
          {/* Liked tab removed - no likes API endpoint yet */}
          <button 
        ...
        <div className="tab-content">
          ...
          {/* Liked section removed - no likes API endpoint yet */}
          ...
          {activeTab === 'about' && (
            <div className="about-section">
              ...
              <div className="about-content">
                ...
                {userProfile?.website_url && (
                  <p><strong>Website:</strong> <a href={userProfile.website_url} target="_blank" rel="noopener noreferrer">{userProfile.website_url}</a></p>
                )}
                {userProfile?.instagram_url && (
                  <p><strong>Instagram:</strong> <a href={userProfile.instagram_url} target="_blank" rel="noopener noreferrer">{userProfile.instagram_url}</a></p>
                )}
